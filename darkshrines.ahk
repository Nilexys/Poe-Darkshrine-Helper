...
Parse_item(Item)
{
	...
	If(RegExMatch(Item, "i)^\+.* to Evasion Rating$"))
		Result := Result . "`rspawns several groups of monsters (no jewelry)"
	...
	If(RegExMatch(Item, "i)^\+.* to Evasion Rating$"))
		Result := Result . "`r3 rare monsters spawn near the shrine (jewelry only)"
	...
	;##### one random monster drops extra items
	...
	If(RegExMatch(Item, "i)^Monsters reflect .*% of Elemental Damage$"))
		Result := Result . "`rchanges loot allocation to free for all?"
...
ParseAffix(Item)
{
	...
	If(RegExMatch(Item, "i)^\+.* to Evasion Rating$"))
		Result := Result . "`rBeware those who lie in ambush`r    spawns several groups of monsters (no jewelry)"
	...
	If(RegExMatch(Item, "i)^\+.* to Evasion Rating$"))
		Result := Result . "`rAlways watch your back`r    3 rare monsters spawn near the shrine (jewelry only)"
	...
	If(RegExMatch(Item, "i)^Mirrored$"))
		Result := Result . "`rThe little things add up`r    unknown"

	If(RegExMatch(Item, "i)^.*% increased Flask effect duration$"))
		Result := Result . "`rAtziri's power fills you`r    unknown (possibly fills Vaal skills?)"

	If(RegExMatch(Item, "i)^Monsters reflect .*% of Elemental Damage$"))
		Result := Result . "`rRewards are seized only by those quick enough to find them`r    changes loot allocation to free for all?"
	...
	If(RegExMatch(Item, "i)^Area is inhabited by ranged monsters$"))
		Result := Result . "`rCareful now`r    unknown"

	If(RegExMatch(Item, "i)^Area is inhabited by Animals$"))
		Result := Result . "`rUnflinching`r    unknown"
