Parse_item(Item)
{
	...
	If(RegExMatch(Item, "i)^\+.* to Level of Socketed Minion Gems$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rartisan's strongbox"
	}
	...
	If(RegExMatch(Item, "i)^.*% increased Flask Mana Recovery Rate$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rchemist's strongbox"
	}
	...
	If(RegExMatch(Item, "i)^.*% increased Damage$")){
		Importance := Importance> 7 ? Importance: 7
	...
	If(RegExMatch(Item, "i)^Adds .*-.* Chaos Damage$")){
		Importance := Importance> 8 ? Importance: 8
	...
	If(RegExMatch(Item, "i)^.*% reduced Enemy Block Chance$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rBattering tempest"
	}
	...
	If(RegExMatch(Item, "i)^.*% increased Armour, Evasion and Energy Shield$")){
		Importance := Importance> 3 ? Importance: 3
	...
	If(RegExMatch(Item, "i)^.*% of Fire Damage Leeched as Life$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rdrops 20 Chance Orbs"
	}

	If(RegExMatch(Item, "i)^.*% of Cold Damage Leeched as Life$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rdrops 20 Chance Orbs"
	}

	If(RegExMatch(Item, "i)^.*% of Lightning Damage Leeched as Life$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rdrops 20 Chance Orbs"
	}
	...
	If(RegExMatch(Item, "i)^.*% increased Critical Strike Multiplier with Elemental Skills$")){
		Importance := Importance> 8 ? Importance: 8
		Result := Result . "`rdrops a Divine Orb"
	}
	...
	If(RegExMatch(Item, "i)^.*% faster start of Energy Shield Recharge$")){
		Result := Result . "`rdrops a Divination Card"
	}
	...
	If(RegExMatch(Item, "i)^.*% increased Critical Strike Chance with Two Handed Melee Weapons$")){
	...
	If(RegExMatch(Item, "i)^.*% increased Totem Life$")){
		Importance := Importance> 6 ? Importance: 6
