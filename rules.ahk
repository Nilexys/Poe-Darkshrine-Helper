Parse_item(Item)
{
	...
	If(RegExMatch(Item, "i)^.{1,3}% reduced Enemy Stun Threshold$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rpermanent massive shrine buff"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Attack Speed$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rpermanent acceleration shrine"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to maximum Life$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rpermanent diamond shrine"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% Chance to Block$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rpermanent impenetrable shrine"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% Chance to Block with Shields$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rpermanent impenetrable shrine"
	}

	If(RegExMatch(Item, "i)^.{1,3}% additional Chance to Block with Staves$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rpermanent impenetrable shrine"
	}

	If(RegExMatch(Item, "i)^.{1,3}% additional Block Chance while Dual Wielding$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rpermanent impenetrable shrine"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} Mana Gained on Kill$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rpermanent replenishing shrine"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Monster Movement Speed$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rpermanent divine shrine"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Monster Attack Speed$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rpermanent divine shrine"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Monster Cast Speed$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rpermanent divine shrine"
	}

	If(RegExMatch(Item, "i)^Monsters have .{1,3}% increased Area of Effect$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rpermanent echoing shrine"
	}

	If RegExMatch(Type, "i)(jewel)")
	If(RegExMatch(Item, "i)^.{1,3}% increased Accuracy Rating$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rpermanent shrouded shrine"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Melee Damage$")){
		Importance := Importance> 6 ? Importance: 6
	...
	If(RegExMatch(Item, "i)^Players Recover Life, Mana and Energy Shield .{1,3}% slower$")){
	...
	If(RegExMatch(Item, "i)^\+.{1,3} to Level of Socketed Melee Gems$")){
		Result := Result . "`rlarge strongbox"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Physical Damage$")){
		Result := Result . "`rblacksmith's strongbox"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Level of Socketed Bow Gems$")){
		Result := Result . "`rblacksmith's strongbox"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Stun Recovery$")){
		Result := Result . "`rarmourer's strongbox"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Level of Socketed Lightning Gems$")){
		Result := Result . "`rornate strongbox"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Physical Damage with .*$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rartisan's strongbox"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Physical Weapon Damage while .*$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rartisan's strongbox"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Level of Socketed Minion Gems$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rartisan's strongbox"
	}

	If(RegExMatch(Item, "i)^.{1,3}% additional Chance to Block Spells (with|while) .*$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rarcanist's strongbox"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Level of Socketed Fire Gems$")){
		Result := Result . "`rgemcutter's strongbox"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Flask Life Recovery Rate$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rchemist's strongbox"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Flask Mana Recovery Rate$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rchemist's strongbox"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Level of Socketed Cold Gems$")){
		Importance := Importance> 6 ? Importance: 6
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Damage$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rPerandus Bank (unique jeweller's strongbox)"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Damage over Time$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rPerandus Bank (unique jeweller's strongbox)"
	}
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Monster Damage$")){
		Importance := Importance> 7 ? Importance: 7
	...
	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Lightning Damage$")){
		Result := Result . "`rGalvanizing tempest"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Lightning Damage to Attacks$")){
		Result := Result . "`rGalvanizing tempest"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Lightning Damage to Spells$")){
		Result := Result . "`rGalvanizing tempest"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Fire Damage$")){
		Result := Result . "`rSeething tempest"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Fire Damage to Attacks$")){
		Result := Result . "`rSeething tempest"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Fire Damage to Spells$")){
		Result := Result . "`rSeething tempest"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Cold Damage$")){
		Result := Result . "`rShivering tempest"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Cold Damage to Attacks$")){
		Result := Result . "`rShivering tempest"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Cold Damage to Spells$")){
		Result := Result . "`rShivering tempest"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Chaos Damage$")){
		Importance := Importance> 8 ? Importance: 8
		Result := Result . "`rCorrupting tempest"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% Monster Physical Damage Reduction$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rBattering tempest"
	}

	If(RegExMatch(Item, "i)^.{1,3}% reduced Enemy Block Chance$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rBattering tempest"
	}
	...
	If(RegExMatch(Item, "i)^\+.{1,3}% Monster Fire Resistance$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rMolten tempest"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% Monster Lightning Resistance$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rElectrocuting tempest"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% Monster Cold Resistance$")){
		Importance := Importance> 4 ? Importance: 4
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Evasion and Energy Shield$")){
		Result := Result . "`rinvasion boss"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to maximum Energy Shield$")){
		Result := Result . "`rduplicates all rare monsters"
	}

	If not RegExMatch(Type, "i)(ring|amulet)")
	If(RegExMatch(Item, "i)^\+.{1,3} to Evasion Rating$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rspawns several groups of monsters"
	}

	If not RegExMatch(Type, "i)(jewel)")
	If(RegExMatch(Item, "i)^.{1,3}% increased Evasion Rating$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rspawns several groups of monsters"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Armour and Evasion$")){
		Importance := Importance> 4 ? Importance: 4
	...
	If(RegExMatch(Item, "i)^.{1,3}% of Physical Attack Damage Leeched as Life$")){
		Importance := Importance> 4 ? Importance: 4
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Armour and Energy Shield$")){
		Importance := Importance> 3 ? Importance: 3
		Result := Result . "`rspawns 4 Kaom's totems"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Attack Speed (with|while) .*$")){
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Armour, Evasion and Energy Shield$")){
		Importance := Importance> 3 ? Importance: 3
	...
	If(RegExMatch(Item, "i)^\+.{1,3} to All Attributes$")){
		Result := Result . "`rspawns a random rogue exile"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Strength$")){
		Result := Result . "`rspawns a marauder rogue exile"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Dexterity$")){
		Result := Result . "`rspawns a ranger rogue exile"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Intelligence$")){
		Result := Result . "`rspawns a witch rogue exile"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Dexterity and Intelligence$")){
		Result := Result . "`rspawns a shadow rogue exile"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Strength and Intelligence$")){
		Result := Result . "`rspawns a templar rogue exile"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Strength and Dexterity$")){
	...
	If(RegExMatch(Item, "i)^\+.{1,3}% to all Elemental Resistances$")){
		Result := Result . "`rspawns multiple warbands"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% to Lightning Resistance$")){
		Result := Result . "`rspawns one group of Brinerots"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% to Cold Resistance$")){
		Result := Result . "`rspawns one group of Mutewinds"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% to Fire Resistance$")){
		Result := Result . "`rspawns one group of Redblades"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% to Chaos Resistance$")){
	...
	If(RegExMatch(Item, "i)^Monsters deal .{1,3}% extra Damage as Lightning$")){
		Result := Result . "`rspawns a Tormented Storm Cultist"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to maximum Mana$")){
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Mana Regeneration Rate$")){
		Result := Result . "`rspawns a Tormented Emblezzler"
	}

	If(RegExMatch(Item, "i)^Monsters deal .{1,3}% extra Damage as Cold$")){
		Result := Result . "`rspawns a Tormented Aurora Cultist"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Cast Speed while .*$")){
		Result := Result . "`rspawns a Tormented Forger"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% to Fire and Cold Resistances$")){
		Result := Result . "`rspawns a Tormented Vaal Cultist"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% to Fire and Lightning Resistances$")){
		Result := Result . "`rspawns a Tormented Vaal Cultist"
	}

	If(RegExMatch(Item, "i)^\+.{1,3}% to Cold and Lightning Resistances$")){
		Result := Result . "`rspawns a Tormented Vaal Cultist"
	}

	If(RegExMatch(Item, "i)^Unique Boss deals .{1,3}% increased Damage$")){
		Result := Result . "`rspawns a Tormented Seditionist"
	}

	If(RegExMatch(Item, "i)^Unique Boss has .{1,3}% increased Attack and Cast Speed$")){
		Result := Result . "`rspawns a Tormented Seditionist"
	}
	...
	If(RegExMatch(Item, "i)^Monsters deal .{1,3}% extra Damage as Fire$")){
		Result := Result . "`rspawns a Tormented Arsonist"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Attack and Cast Speed$")){
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Cast Speed$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rAll unequipped items are scoured and you are rewarded with alchemy/transmute orbs"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Chance$")){
		Result := Result . "`rone white item in your inventory is turned to rare"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Global Critical Strike Chance$")){
		Result := Result . "`rone white item in your inventory is turned to rare"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Global Critical Strike Multiplier$")){
		Importance := Importance> 7 ? Importance: 7
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Light Radius$")){
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Freeze Duration on Enemies$")){
		Result := Result . "`rdrops 40 Armourer's Scraps"
	}

	If(RegExMatch(Item, "i)^.{1,3}% chance to Freeze$")){
		Result := Result . "`rdrops 40 Armourer's Scraps"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Ignite Duration on Enemies$")){
		Result := Result . "`rdrops 20 Blacksmith's Whetstones"
	}

	If(RegExMatch(Item, "i)^.{1,3}% chance to Ignite$")){
		Result := Result . "`rdrops 20 Blacksmith's Whetstones"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Maximum Number of Skeletons$")){
		Result := Result . "`rdrops 40 Orbs of Transmutation"
	}

	If(RegExMatch(Item, "i)^.{1,3}% of Fire Damage Leeched as Life$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rdrops 20 Chance Orbs"
	}

	If(RegExMatch(Item, "i)^.{1,3}% of Cold Damage Leeched as Life$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rdrops 20 Chance Orbs"
	}

	If(RegExMatch(Item, "i)^.{1,3}% of Lightning Damage Leeched as Life$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rdrops 20 Chance Orbs"
	}
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Shock Duration on Enemies$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rdrops 20x Cartographer's Chisels"
	}

	If(RegExMatch(Item, "i)^.{1,3}% chance to Shock$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rdrops 20x Cartographer's Chisels"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Chance with Elemental Skills$")){
		Importance := Importance> 8 ? Importance: 8
		Result := Result . "`rdrops a Divine Orb"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Multiplier with Elemental Skills$")){
		Importance := Importance> 8 ? Importance: 8
		Result := Result . "`rdrops a Divine Orb"
	}

	If(RegExMatch(Item, "i)^.{1,3}% more Monster Life$")){
		Importance := Importance> 6 ? Importance: 6
	...
	If(RegExMatch(Item, "i)^\+.{1,3} Energy Shield gained for each Enemy hit by your Attacks$")){
		Result := Result . "`rdrops a Divination Card"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Energy Shield Recharge Rate$")){
		Result := Result . "`rdrops a Divination Card"
	}

	If(RegExMatch(Item, "i)^.{1,3}% faster start of Energy Shield Recharge$")){
		Result := Result . "`rdrops a Divination Card"
	}

	If(RegExMatch(Item, "i)^.{1,3}% more Rare Monsters$")){
		Importance := Importance> 6 ? Importance: 6
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Rarity of Items found$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rfills inventory with scroll fragments"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Spell Damage while .*$")){
		Importance := Importance> 4 ? Importance: 4
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Multiplier with One Handed Melee Weapons$")){
		Result := Result . "`rone mob drops 5 rare daggers"
	}

	If not RegExMatch(Type, "i)(jewel)")
	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Chance for Spells$")){
		Result := Result . "`rone mob drops 5 rare staves"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Chance with Two Handed Melee Weapons$")){
		Result := Result . "`rone mob drops 5 rare maces"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Melee Critical Strike Multiplier$")){
		Result := Result . "`rone mob drops 5 rare maces"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Melee Critical Strike Chance$")){
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Chance with One Handed Melee Weapons$")){
		Result := Result . "`rone mob drops 5 rare claws"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Cast Speed with Fire Skills$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rone mob drops 5 rare amulets"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Chance with Fire Skills$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rone mob drops 5 rare amulets"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Multiplier with Fire Skills$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rone mob drops 5 rare amulets"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Projectile Damage$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rrandom mob drops 5 rare jewels"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Cast Speed with Cold Skills$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rrandom mob drops 5 rare rings"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Chance with Cold Skills$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rrandom mob drops 5 rare rings"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Multiplier with Cold Skills$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rrandom mob drops 5 rare rings"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Cast Speed with Lightning Skills$")){
		Result := Result . "`rrandom mob drops 5 rare belts"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Chance with Lightning Skills$")){
		Result := Result . "`rrandom mob drops 5 rare belts"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Multiplier with Lightning Skills$")){
		Result := Result . "`rrandom mob drops 5 rare belts"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Melee Physical Damage while holding a Shield$")){
		Result := Result . "`rrandom mob drops 5 rare shields"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Multiplier with Two Handed Melee Weapons$")){
		Result := Result . "`rrandom mob drops 5 rare bows"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Weapon Critical Strike Chance while Dual Wielding$")){
		Result := Result . "`rrandom mob drops 5 rare swords"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Multiplier while Dual Wielding$")){
		Result := Result . "`rrandom mob drops 5 rare two-handed swords"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Critical Strike Multiplier for Spells$")){
		Result := Result . "`rrandom mob drops 5 rare sceptres"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Trap Damage$")){
		Result := Result . "`rrandom mob drops 10 rare items"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Trap Throwing Speed$")){
		Result := Result . "`rrandom mob drops 10 rare items"
	}

	If(RegExMatch(Item, "i)^.{1,3}% chance to Knock Enemies Back on hit$")){
		Result := Result . "`rrandom mob drops lots of wisdom scrolls"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Totem Life$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rrandom mob drops a large amount of currency"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Totem Damage$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rrandom mob drops a large amount of currency"
	}

	If(RegExMatch(Item, "i)^Totems gain \+.{1,3}% to all Elemental Resistances$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rrandom mob drops a large amount of currency"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Mine Laying Speed$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rmobs have a chance to drop sacrifice fragments"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Mine Damage$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rmobs have a chance to drop sacrifice fragments"
	}
	...
	If(RegExMatch(Item, "i)^Monsters reflect .{1,3}% of Elemental Damage$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rrare mobs killed within the next ~20 seconds drop a unique"
	}

	If(RegExMatch(Item, "i)^Monsters reflect .{1,3}% of Physical Damage$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rrare mobs killed within the next ~20 seconds drop a unique"
	}
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Energy Shield$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rdoubles the size of magic monster packs"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased maximum Energy Shield$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rdoubles the size of magic monster packs"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Armour$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rRare Monsters each have a Nemesis Mod"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Chaos Damage$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rMonsters Poison on Hit"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Chaos Damage to Attacks$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rMonsters Poison on Hit"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Physical Damage$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rMonsters Fracture"
	}

	If(RegExMatch(Item, "i)^Adds .{1,3}-.{1,3} Physical Damage to Attacks$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rMonsters Fracture"
	}
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Cold Damage$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rMonsters deal 90-110% extra Damage as Cold"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Fire Damage$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rMonsters deal 90-110% extra Damage as Fire"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Lightning Damage$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rMonsters deal 90-110% extra Damage as Lightning"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Accuracy Rating$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rMonsters fire 4 additional Projectiles"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Projectile Speed$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rMonsters' skills Chain 2 additional times"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} Life gained on Kill$")){
		Importance := Importance> 4 ? Importance: 4
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Armour$")){
		Importance := Importance> 4 ? Importance: 4
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased maximum Mana$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`rPlayers Recover Life, Mana and Energy Shield 60% slower"
	}

	If(RegExMatch(Item, "i)^.{1,3}% reduced Mana Cost of Skills$")){
		Importance := Importance> 4 ? Importance: 4
		Result := Result . "`r-(12-16)% maximum Player Resistances"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} Life gained for each Enemy hit by Attacks$")){
		Result := Result . "`rKill streaks award you with Rampage bonuses"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} Life gained for each Enemy hit by your Attacks$")){
		Result := Result . "`rKill streaks award you with Rampage bonuses"
	}
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased maximum Life$")){
		Result := Result . "`rcheck all clickables in the area"
	}

	If(RegExMatch(Item, "i)^Unique Boss has .{1,3}% increased Life$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`revery enemy's health is reduced to 1"
	}

	If(RegExMatch(Item, "i)^Unique Boss has .{1,3}% increased Area of Effect$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`revery enemy's health is reduced to 1"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Elemental Damage with Weapons$")){
		Result := Result . "`rall weapons will drop with 20% quality"
	}

	If(RegExMatch(Item, "i)^Minions deal .{1,3}% increased Damage$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rall enemies are immobilized"
	}

	If(RegExMatch(Item, "i)^Minions have .{1,3}% Chance to Block$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rall enemies are immobilized"
	}

	If(RegExMatch(Item, "i)^Minions have .{1,3}% increased maximum Life$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rall enemies are immobilized"
	}

	If(RegExMatch(Item, "i)^Minions have \+.{1,3}% to all Elemental Resistances$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rall enemies are immobilized"
	}

	If(RegExMatch(Item, "i)^-.{1,3}% maximum Player Resistances$")){
		Importance := Importance> 8 ? Importance: 8
	...
	If(RegExMatch(Item, "i)^.{1,3}% more Magic Monsters$")){
		Importance := Importance> 4 ? Importance: 4
	...
	If(RegExMatch(Item, "i)^\+.{1,3} Mana gained for each Enemy hit by your Attacks$")){
	...
	If(RegExMatch(Item, "i)^\+.{1,3}% to Quality of Socketed Support Gems$")){
		Importance := Importance> 6 ? Importance: 6
	...
	If(RegExMatch(Item, "i)^-.{1,3} to Mana Cost of Skills$")){
		Importance := Importance> 6 ? Importance: 6
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Life Leeched per second$")){
		Importance := Importance> 6 ? Importance: 6
	...
	If(RegExMatch(Item, "i)^Monsters gain .{1,3} Endurance Charge(s)? every 20 seconds$")){
		Importance := Importance> 3 ? Importance: 3
		Result := Result . "`rclears all your vendors"
	}

	If(RegExMatch(Item, "i)^Monsters gain .{1,3} Power Charge(s)? every 20 seconds$")){
		Importance := Importance> 3 ? Importance: 3
		Result := Result . "`rclears all your vendors"
	}

	If(RegExMatch(Item, "i)^Monsters gain .{1,3} Frenzy Charge(s)? every 20 seconds$")){
		Importance := Importance> 3 ? Importance: 3
		Result := Result . "`rclears all your vendors"
	}

	If(RegExMatch(Item, "i)^.{1,3} Life Regenerated per second$")){
		Result := Result . "`rrefills flasks"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Flask Charges gained$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rno mana costs"
	}

	If(RegExMatch(Item, "i)^.{1,3}% reduced Flask Charges used$")){
		Importance := Importance> 6 ? Importance: 6
		Result := Result . "`rno mana costs"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Area Damage$")){
		Result := Result . "`rsockets a new random jewel in your passive tree (even in unallocated nodes)"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Stun Duration on Enemies$")){
	...
	If(RegExMatch(Item, "i)^.{1,3}% reduced Attribute Requirements$")){
		Result := Result . "`rthe item you sacrificed appears in the next shrine"
	}

	If(RegExMatch(Item, "i)^.{1,3}% increased Movement Speed$")){
		Result := Result . "`rgain sacrificed item stats"
	}

	If(RegExMatch(Item, "i)^Reflects .{1,3} Physical Damage to Melee Attackers$")){
		Result := Result . "`rheadhunter effect (get powers from slain enemies)"
	}

	If(RegExMatch(Item, "i)^.{1,3}% of Physical Attack Damage Leeched as Mana$")){
		Result := Result . "`rputs a map into the map device in eternal laboratory"
	}

	If(RegExMatch(Item, "i)^\+.{1,3} to Level of Socketed Gems$")){
		Importance := Importance> 7 ? Importance: 7
		Result := Result . "`rresets all dailies"
	}

	If(RegExMatch(Item, "i)^Monsters fire .{1,3} additional Projectiles$")){
		Importance := Importance> 6 ? Importance: 6
	...
	If(RegExMatch(Item, "i)^Monsters have .{1,3}% increased Critical Strike Chance$")){
		Result := Result . "`rempties all flasks"
	}

	If(RegExMatch(Item, "i)^Monsters have .{1,3}% increased Critical Strike Multiplier$")){
		Result := Result . "`rempties all flasks"
	}

	If(RegExMatch(Item, "i)^Monsters have .{1,3}% chance to Avoid Elemental Status Ailments$")){
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Spell Damage$")){
	...
	If(RegExMatch(Item, "i)^.{1,3}% increased Flask effect duration$")){
